...
import { DistributionParamInput } from './data/DistributionParamInput';
import { DataInputTabButton } from './data/DataInputTabButton';

interface DataInputSectionProps {
  inputMethod: string;
  setInputMethod: (method: string) => void;
  handleFileUpload: (e: React.ChangeEvent<HTMLInputElement>) => void;
  handleDistributionGenerate: (type: string, params?: any) => void;
...
const DataInputSection: React.FC<DataInputSectionProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* 输入方法切换 */}
      <div className="data-input-tabs-container" style={{ marginBottom: 24 }}>
        <div className="data-input-tabs-wrapper">
          <DataInputTabButton
            icon={<UploadOutlined />}
            label={t('data.upload')}
          ...
          <DataInputTabButton
            ...
            label={t('data.distribution')}
            isActive={inputMethod === 'distribution'}
            onClick={() => setInputMethod('distribution')}
            color="#66d9ef"
          />
          <DataInputTabButton
            icon={<RobotOutlined />}
            label={t('data.aiGenerate')}
            isActive={inputMethod === 'ai'}
      ...
      {inputMethod === 'upload' && (
        <Row justify="center">
          <Col xs={24} lg={16}>
            ...
            {/* 数据导入成功状态提示 - 只在用户上传时显示 */}
            {isUserUploadedData && !isAutoGenerated && data.length > 0 && inputMethod === 'upload' && (
              <Alert
                className="success-pulse-alert"
                message={
                  <Space>
      ...
      {inputMethod === 'distribution' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          {selectedDistribution && (
            <DistributionParamInput
              distributionType={selectedDistribution}
              onSubmit={(params, sampleSize) => {
                handleDistributionGenerate(selectedDistribution, { ...params, sampleSize });
                setSelectedDistribution(null);
              }}
              onCancel={() => setSelectedDistribution(null)}
            />
          )}
          ...
          {/* 生成成功提示 - 只在用户手动生成时显示 */}
          {isUserUploadedData && !isAutoGenerated && data.length > 0 && inputMethod === 'distribution' && (
            <Alert
              className="success-pulse-alert"
      ...
      {inputMethod === 'ai' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          {/* AI生成成功提示 - 只在用户手动生成时显示 */}
          {isUserUploadedData && !isAutoGenerated && data.length > 0 && inputMethod === 'ai' && (
            <Alert
              className="success-pulse-alert"
              message={
                <Space>
